...
if (!import.meta.env.VITE_XERO_OAUTH_CLIENT_ID) {
	throw new Error(
		"Please set the PUBLIC_XERO_OAUTH_CLIENT_ID environment variable.",
	);
}
...
const discoverConfig = once(() =>
	discovery(new URL(CONFIG.discoveryUrl), CONFIG.clientId),
);

const launchWebAuthFlow = (
	details: Browser.identity.WebAuthFlowDetails,
): Promise<string> =>
...
async function getTokens() {
	const { tokens } = await browser.storage.local.get<{ tokens: Tokens }>(
		"tokens",
	);
	return tokens;
}
